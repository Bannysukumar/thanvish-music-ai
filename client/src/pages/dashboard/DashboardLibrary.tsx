...
import { Library, Play, Trash2, Search, Music2, User } from "lucide-react";
...
export default function DashboardLibrary() {
  ...
  const loadCompositions = async () => {
    ...
    try {
      setIsLoading(true);
      // Fetch all music generated by the current user
      const musicQuery = query(
        collection(db, "generatedMusic"),
        where("generatedBy.userId", "==", user.id),
   ...
   */
  const handlePlayPause = (composition: GeneratedMusic) => {
          ...
          {displayCompositions.map((composition) => (
            <Card key={composition.id} className="flex flex-col" onContextMenu={handleContextMenu}>
              ...
              <CardContent className="flex-1 flex flex-col space-y-4">
                ...
                {/* Hidden audio element with download prevention */}
                <audio
